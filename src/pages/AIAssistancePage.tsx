...
export function AIAssistancePage() {
  ...
  const clients = useMemo<ClientOption[]>(() => {
    const docs = clientsData?.response?.data?.docs ?? [];
        ...
        <div className="p-3 bg-white border-t border-border/50">
          <form onSubmit={handleSend} className="flex gap-2">
            <div className="relative flex-1">
              <Input
                placeholder="Ask me anything... Use @ to mention a client"
                value={input}
          ...
          {requestError && (
            <div className="mt-2 text-xs text-red-600">
              {(requestError as any)?.data?.message || 'Unable to reach the AI assistant right now.'}
            </div>
          )}
        </div>
      </Card>
