...
export function RolesPage() {
  ...
  const [patchClinicPermissions, { isLoading: isSavingPermissions }] =
    usePatchClinicPermissionsMutation();
  const clinic = clinicResponse?.response?.data;

  // Calculate dynamic counts
  const clinicianCount =
    clinic?.members?.filter((m) => m.role === "clinician").length || 0;
  const adminCount =
    clinic?.members?.filter(
  ...
  const roles = [
    {
      ...
      users: adminCount,
      access: "all",
      desc: "Full system control and access to all data.",
    },
    {
      id: 2,
      name: "Clinician",
      users: clinicianCount,
      access: "Restricted",
      desc: "Can only access pages enabled below.",
    },
  ];
  ...
  const clinicianInitialPermissions = useMemo(
    () =>
      PERMISSION_KEYS.filter((p) => clinicianHasPermission(p.id)).map(
        (p) => p.id,
      ),
    [clinic?.permissions],
  );
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {roles.map((role) => (
          <Card
            key={role.id}
            className={`shadow-md hover:shadow-lg transition-all ${role.name === "Admin" ? "border-primary/20 bg-primary/5" : ""}`}
          >
            <CardHeader>
                  ...
                  {role.name === "Admin" ? (
                    <>
                      <li className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-green-600" /> Full access
                        to specific dashboards
                      </li>
                      <li className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-green-600" /> Manage all
                        billing & subscriptions
                      </li>
                      <li className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-green-600" /> User & Role
                        management
                      </li>
                    </>
                  ) : (
                    <>
                      ...
                      <li className="flex items-center gap-2 text-muted-foreground">
                        <Lock className="h-3 w-3" /> No access to restricted
                        areas
                      </li>
                    </>
                  )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Detailed permission breakdown</CardDescription>
        </CardHeader>
        <CardContent>
          {clinicLoading && (
            <div className="text-sm text-muted-foreground">
              Loading clinic permissions...
            </div>
          )}
          {clinicError && (
            <div className="text-sm text-destructive">
              Failed to load clinic permissions.
            </div>
          )}
      ...
      {selectedRole && (
        <ManagePermissionsModal
          ...
          initialPermissions={
            selectedRole === "Clinician" ? clinicianInitialPermissions : []
          }
