...
type TabKey = 'about' | 'sample' | 'results';
      ...
      <main id="assessment-content" className="space-y-6 animate-in fade-in duration-500 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pt-14">
        ...
        <div className="flex gap-2 border-b border-border/40 overflow-x-auto no-scrollbar">
          {[
            ...
            { key: 'sample' as const, label: 'assessment' },
            ...
            >
              {tab.label}
              {activeTab === tab.key && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-t-full" />}
            </button>
          ))}
        </div>
                ...
                <div className="space-y-10">
                  <div className="p-6 bg-muted/20 rounded-xl border border-border/20 relative group overflow-hidden">
                    ...
                    <p className="text-foreground leading-relaxed text-base font-medium italic relative z-10">
                      "{template.description || instance.note || 'No abstract provided.'}"
                    </p>
                  </div>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-5 duration-700">
                {currentStep === -1 ? (
                  ...
                  <div className="max-w-2xl mx-auto p-12 sm:p-24 text-center space-y-8 animate-in zoom-in duration-500">
                    ...
                    <Button size="lg" className="h-14 px-8 rounded-xl font-bold shadow-lg shadow-primary/20" onClick={() => setActiveTab('results')}>
                      See your Results
                    </Button>
                  </div>
                ) : (
            ...
            {activeTab === 'results' && (
              <div className="animate-in fade-in slide-in-from-bottom-5 duration-700">
                <div className="p-8 border-b border-border/40 bg-white">
                  <h2 className="text-xl font-bold text-foreground">Assessment Results: {template.title}</h2>
                ...
                <div className="p-8 space-y-12">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-secondary p-6 sm:p-8 rounded-3xl border border-primary/10 relative overflow-hidden">
                      <div className="lg:col-span-3 flex flex-col items-center gap-4">
                        <div className="relative h-32 w-32 sm:h-40 sm:w-40 rounded-full bg-white flex flex-col items-center justify-center border-[3px] border-primary/20 shadow-xl">
                          ...
                          <span className="text-4xl sm:text-6xl font-black text-foreground">{instance.score ?? calculateScore()}</span>
                        </div>
                        <div className="px-4 py-1 rounded-full bg-primary/10 border border-primary/20">
                          <span className="text-[10px] font-bold text-primary uppercase tracking-wider capitalize">{instance.status || 'pending'}</span>
                        </div>
                      </div>
                      <div className="lg:col-span-9">
                        <p className="text-muted-foreground">Max Score: <span className="font-bold text-foreground">{instance.maxScore ?? questions.length}</span></p>
                        <p className="text-muted-foreground mt-2">
                          Completed At:{' '}
                          <span className="font-bold text-foreground">
                            {instance.completedAt ? new Date(instance.completedAt).toLocaleString() : 'Pending'}
                          </span>
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="rounded-xl border border-border/40 overflow-hidden bg-white shadow-sm overflow-x-auto">
                      <table className="w-full text-sm text-left">
                        <thead className="bg-muted/30 border-b border-border/40 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                          <tr>
                            ...
                            <th className="px-6 py-4 text-center w-40">Response</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-border/40 font-medium">
                          {questions.map((q, idx) => (
                            <tr key={q.id || idx} className="hover:bg-muted/5 transition-colors">
                              ...
                              <td className="px-6 py-4 text-center">
                                <div className={cn('px-3 py-1 rounded-md text-[10px] font-bold uppercase border bg-primary/10 border-primary/20 text-primary')}>
                                  {String(mergedAnswers[q.id || ''] ?? 'N/A')}
                                </div>
                              </td>
                            </tr>
                          ))}
